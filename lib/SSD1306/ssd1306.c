...
bool ssd1306_init(gpio_num_t sda_pin, gpio_num_t scl_pin, uint32_t frequency){
    esp_err_t esp_err;

    i2c_master_init(sda_pin, scl_pin, frequency);
    ...
    if (esp_err == ESP_OK){
        ESP_LOGI(tag, "Display configurado com sucesso");
        return true;
    }

    ESP_LOGE(tag, "Falha na configuração do display: 0x%.2X", esp_err);
    return false;
}
